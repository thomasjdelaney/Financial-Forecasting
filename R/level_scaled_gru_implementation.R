...
data_gen <- function(dataset) {
  ...
  for (i in 1:num_series){
    ...
    level_sample <- ets_model$states[,1][2:(sample_points+1)]
    level_value <- ets_model$states[,1][(sample_points+2):(sample_points+1+yearly_forecast_horizon_length)]
...
for (i in 1:num_test_series){
  actual_preds[i,] <- getPredictionFromLevelParams(preds[i,], test_levels[i,], test_alphas[[i]])[[1]]
}
